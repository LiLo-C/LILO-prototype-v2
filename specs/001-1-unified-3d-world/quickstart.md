...
scenarios still pass unmodified (SC-009) — this guide only adds what changed.

## Prerequisites

- Same as [001's quickstart](../001-core-prototype/quickstart.md): Xcode with iOS 26 SDK, a
  physical iPhone 17 for fps checks (SC-002), the `v2` scheme.
- Before starting: confirm 001's existing `v2Tests` suite (`GameConfigTests`,
  `BatteryControllerTests`, `LightStateTests`) still builds and passes green — this is SC-009's
...
### 1. The flashlight lights the room (User Story 1, SC-001)
...
- Stand still at spawn with a full charge → floor, nearby walls, the desk, and the player
  ...
  every screen edge (SC-001 — take a screenshot here for the record).
...
### 2. Light states read as one light (User Story 2, SC-004)
...
- **Flickering**: watch for distinct dip-and-recover events with steady light between them — not
  ...
  "flickering" or "failing", not "glitching"/"noise" (SC-004).
...
### 3. The room stays navigable in the dark (User Story 3, SC-006)

- Set charge to 0% via a debug hook or by waiting it out. Hand the device to someone outside the
  dev team with no instructions → they reach the door within 60 seconds (SC-006).
...
- Walk diagonally into a wall, then into the desk, at several angles → the character slides
  along the surface every time rather than stopping dead or clipping through (SC-007 — repeat at
  least 10 times across both surfaces and count any failures).
- Touch anywhere inside the left half of the screen with no joystick visible → it appears
...
### 5. Side-by-side feel check (SC-003, SC-005)
...
- Build 001's commit (`c982b79`) and this feature's build on two devices, or record short clips
  ...
  and feels more like a horror game" → at least 2 of 3 pick the 001-1 build (SC-003).
- During the Compact Darkness scenario above, ask observers if anything on screen looks brighter
  than the floor around it → none should say yes (SC-005).

### 6. Performance with shadows (SC-002)
...
- On the physical iPhone 17, move continuously for at least 30 seconds with shadows enabled and
  ...
  sustained ≥60 fps. If this fails, set `GameConfig.shadowsEnabled = false` and/or reduce
  `shadowMapSize`/`shadowSampleCount`, per FR-022's fallback, and re-measure.

### 7. Config-only tuning (SC-008)
...
- Change `flashlightNormalRadius`, `lightRadiusEaseRate`, `wallHeight`, and
  ...
  → each visibly changes behavior with no other file touched (SC-008).
